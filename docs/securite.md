...
Le compte de service principal du projet est :

`foodtrack-ci@form-gke-eleve04-42a1.iam.gserviceaccount.com`

Il possède notamment les rôles :

- `roles/artifactregistry.writer`
- `roles/container.developer`

Ce compte est utilisé par le pipeline CI/CD.

La configuration du node pool a également permis de confirmer que
`foodtrack-ci` est utilisé par les nœuds GKE.

Le même compte sert donc actuellement à deux usages différents.

Une amélioration possible serait de créer un compte de service dédié aux
nœuds GKE afin de mieux séparer les permissions et respecter le principe
du moindre privilège.

Le compte Compute Engine par défaut possède encore le rôle `Editor`.
...
Vérifier que les rôles IAM attribués aux utilisateurs et aux comptes de service
sont adaptés à leur usage.

### État actuel

Les droits IAM des quatre membres de l'équipe ont été vérifiés.

Plusieurs rôles permettent d'administrer le projet, notamment `Éditeur`,
`Administrateur de projet IAM` et `Administrateur de compte de service`.

Ces droits ont été attribués dans le cadre de l'environnement de formation
et permettent aux membres de l'équipe d'administrer le projet.

## Workload Identity Federation

### Objectif

Éviter l'utilisation de clés JSON permanentes pour authentifier GitHub Actions
...
Vérifier que les règles de pare-feu autorisent uniquement les flux nécessaires.
...
L'authentification par mot de passe est désactivée afin de conserver
...
Vérifier comment le plan de contrôle du cluster GKE est exposé
et identifier les risques associés.
...
Cette configuration permet au pipeline GitHub Actions d'accéder au cluster
malgré l'utilisation d'adresses IP dynamiques par les runners GitHub.
...
Cette configuration augmente l'exposition réseau mais constitue un compromis
lié au fonctionnement du pipeline CI/CD.

## Secrets

### Objectif

Vérifier qu'aucune information sensible n'est stockée directement
dans le dépôt Git ou dans GitHub Actions.

### État actuel

Aucune clé de service ni secret sensible n'est stocké directement
dans le dépôt ou dans GitHub Actions.

Les secrets applicatifs, comme le mot de passe du cache et le jeton de l'API,
sont gérés avec des objets `Secret` Kubernetes.
...
Vérifier que les images Docker utilisées sont identifiées,
...
Elles utilisent le SHA du commit comme tag, ce qui permet d'identifier
précisément la version déployée.
...
Trivy a identifié deux problèmes de niveau HIGH sur les composants API,
portail et cache dans les environnements DEV, TEST et PROD :
...
Ces résultats montrent que les contextes de sécurité des conteneurs
...
Le déploiement en production nécessite donc une validation manuelle
avant exécution.
...
Des améliorations restent possibles, notamment la création d'un compte
de service dédié aux nœuds GKE, le renforcement des contextes de sécurité
Kubernetes et la réduction de l'exposition du plan de contrôle GK
